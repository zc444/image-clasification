...
void test_case_2_classify_seven()
{
  ...
  Image test_img = Image( Vector<int>( digit1_image, img_size ),
                          ncols, nrows );//9
  ...
  HRSTreeSearch clf( 20 );// K = 20 elevel = lgK = 4
  clf.train( img_vec );

  // Check the predicted result
  UTST_ASSERT_CHAR_EQ( clf.classify( test_img ).get_label(), digit1_label );
}
